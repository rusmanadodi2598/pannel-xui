// Package topupsvc tests the FR-06 fee math and gateway seam.
//
// @file      internal/service/topup/topup_test.go
// @for       Quote formula §15.7, min/max range, StubGateway contract.
...
// newTestService mirrors the production defaults (QRIS_FEE_PERCENT / PPN).
func newTestService() *Service {
	return New(StubGateway{}, 0.025, 0.11, 5000, 5000000)
}
...
func TestCreatePayment_GivenStubGateway_ThenAPIUnavailable(t *testing.T) {
	s := newTestService()
	...
	if !errors.Is(err, ErrPaymentAPIUnavailable) {
		t.Fatalf("err = %v, want ErrPaymentAPIUnavailable", err)
	}
}
