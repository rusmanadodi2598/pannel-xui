...
func TestService_PriceOps_GivenPlan_ThenDelegates(t *testing.T) {
	...
	s := New(plans, &fakeUsers{}, &fakeBanner{}, &fakeSender{}, &fakeLocker{}, testLogger())
	ctx := context.Background()

	if got, err := s.ListPlans(ctx); err != nil || len(got) != 1 {
		t.Fatalf("ListPlans = %v, err %v", got, err)
	}
	if err := s.SetPrice(ctx, "ID", 30, 7500); err != nil || !plans.setPrice {
		t.Errorf("SetPrice not delegated: %v", err)
	}
	if err := s.SetEnabled(ctx, "ID", 30, false); err != nil || plans.setEnable == nil {
...
func TestService_BanUser_GivenID_ThenMarkerAndFlagSet(t *testing.T) {
	users := &fakeUsers{}
	banner := &fakeBanner{}
	s := New(&fakePlans{}, users, banner, &fakeSender{}, &fakeLocker{}, testLogger())

	if err := s.BanUser(context.Background(), 42); err != nil {
	...
	if err := s.UnbanUser(context.Background(), 42); err != nil {
